...
const (
	...
	versionCommentKey = "Generated by"
)

// VersionCheckCmd executes the system command "iptables --version" and returns either
...
func VersionCheckCmd(version string) (bool, string, error) {
	// iptables --version
...
// VersionCheckBuffer reads a buffer created by iptables-save to find and report the version
// used in creating the buffer
func VersionCheckBuffer(save [][]byte) string {
	idx := ContainsRulePart(save, versionCommentKey)
	if idx < 0 {
		return ""
	}
	if bytes.Contains(save[idx], []byte(Version1414)) {
		return Version1414
	}
	if bytes.Contains(save[idx], []byte(Version1421)) {
		return Version1421
	}
	if bytes.Contains(save[idx], []byte(Version160)) {
		return Version160
	}
	return ""
}
...
func ContainsRulePart(buf [][]byte, match string) int {
